package com.denmit99.hairbnb.constants;

public final class Constants {
    public static final int LISTING_TITLE_MAX_LENGTH = 100;
    public static final int LISTING_MAX_ADDRESS_LENGTH = 100;
    public static final int LISTING_MAX_ZIPCODE_LENGTH = 20;
    public static final int LISTING_DESCRIPTION_MAX_LENGTH = 500;
    public static final int LISTING_MAX_BATHROOMS = 10;
    public static final int LISTING_MAX_BEDROOMS = 10;
    public static final int LISTING_MAX_BEDS_PER_ROOM = 10;
    public static final int LISTING_MAX_MAX_GUESTS = 50;

    private Constants() {
    }
}

